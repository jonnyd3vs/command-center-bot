...
/**
 * Command to decrement fake player count
 * Restricted to owner-level access
 */
public class RemovePCommand implements Command {

    private final BotConfig botConfig;

    public RemovePCommand(BotConfig botConfig) {
        this.botConfig = botConfig;
    }

    @Override
    public CommandData getCommandData() {
        return Commands.slash("removep", "Decrement the fake player count by 1");
    }
    ...
    public void execute(SlashCommandInteractionEvent event) {
        ...
        try {
            client.removePlayerCount();

            EmbedBuilder embed = new EmbedBuilder()
                    .setTitle("Fake Player Count Decremented")
                    .setColor(Color.BLUE)
                    .addField("Server", serverConfig.getName(), true)
                    .addField("Action", "Decremented by 1", true)
                    .setFooter("Executed by " + event.getUser().getName());

            event.getHook().sendMessageEmbeds(embed.build()).queue();

        } catch (Exception e) {
            event.getHook().sendMessageEmbeds(createErrorEmbed("Failed to decrement player count: " + e.getMessage())).queue();
        } finally {
            client.close();
        }
    }
