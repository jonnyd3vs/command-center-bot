...
public class UnmuteCommand implements Command {
    ...
    public UnmuteCommand(BotConfig botConfig) {
        this.botConfig = botConfig;
    }

    @Override
    public CommandData getCommandData() {
        return Commands.slash("unmute", "Unmute a player in the game")
                .addOption(OptionType.STRING, "username", "The username of the player to unmute", true);
    }
    ...
    public void execute(SlashCommandInteractionEvent event) {
        ...
        try {
            client.unmutePlayer(username);

            EmbedBuilder embed = new EmbedBuilder()
                    .setTitle("Player Unmuted Successfully")
            ...
            event.getHook().sendMessageEmbeds(createErrorEmbed("Failed to unmute player: " + e.getMessage())).queue();
        } finally {
            client.close();
        }
    }
