...
/**
 * Command to clear a player's progress
 */
public class ClearProgressCommand implements Command {

    private final BotConfig botConfig;

    public ClearProgressCommand(BotConfig botConfig) {
        this.botConfig = botConfig;
    }

    @Override
    public CommandData getCommandData() {
        return Commands.slash("clear-progress", "Reset a player's progress")
    ...
    public void execute(SlashCommandInteractionEvent event) {
        ...
        try {
            ...
            EmbedBuilder embed = new EmbedBuilder()
                    .setTitle("Progress Cleared Successfully")
                    .setColor(Color.YELLOW)
                    .addField("Server", serverConfig.getName(), true)
                    .addField("Player", username, true)
                    .setDescription("All progress has been reset for this player.")
                    .setFooter("Executed by " + event.getUser().getName());

            event.getHook().sendMessageEmbeds(embed.build()).queue();

        } catch (Exception e) {
            event.getHook().sendMessageEmbeds(createErrorEmbed("Failed to clear progress: " + e.getMessage())).queue();
        } finally {
            client.close();
        }
    }
